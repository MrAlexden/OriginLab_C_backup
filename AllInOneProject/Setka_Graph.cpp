...
int make_add_data_for_graph_setka(Page & pgLegend,
								  Page & pgData,
								  vector <int> & vI)
{
	if (!pgLegend)
	...
	if (vI.GetSize() == 1)
	{
		...
		v = sheetParamsData.Columns(2).GetDataObject(); /* W */
		...
		v = sheetDiffData.Columns(vI[0]).GetDataObject(); /* Diff */
		...
		dsY.Attach(wksAdd.Columns(2));
		dsY.SetSize(2);
		dsY[0] = dsY[1] = Curve_yfromX(&crv, xatymax(crv) - fwhm(crv, min(vY)) / 2);
		
		/* положение по X левой направляющей */
		dsX.Attach(wksAdd.Columns(3));
		dsX.SetSize(2);
		dsX[0] = dsX[1] = xatymax(crv) - fwhm(crv, min(vY)) / 2;
...
int add_sublines_to_graph_setka(GraphPage & gp,
							    Page & pgLegend)
{
...
{	
	...
	if (gp.Layers(1)) /* Нужно для того, чтобы работало и на сетке и на цилиндре */
		gp.Layers(1).GraphObjects("YR").Text = "Производная";
...
int reprocess_single_segment_setka(int begin,
								   int end,
								   int colnum,
			                       ...
			                       Page & pgOrigData)
{
    ...
    else
    {
    	...
    	if (pgData.GetLongName().Match("Цилиндр*")
    		...
    		resistance = is_signalpeakslookingdown(vOrigSignal) ? -resistance : resistance;
	...
	int err = OriginMakeOne(1, vPila, vSignal, vAdd, vResultData, vFiltedData, vDiffedData, vParameters);
	if (err < 0)
		return err;
	...
	/* Diff Data sheet */
	ds.Attach(sheetDiffData.Columns(colnum));
	ds.SetSize(newSize);
	ds = vDiffedData;
...
int reprocess_setka()
{
	...
	for (i = 0; i < vI.GetSize(); ++i)
	{
		...
		int err = reprocess_single_segment_setka(begin,
										         end,
										         vI[i],
										         sheetOrigData,
										         sheetFitData,
