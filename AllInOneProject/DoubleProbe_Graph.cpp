...
int make_add_data_for_graph_doubleprobe(Page & pgLegend,
								        Page & pgData,
								        vector <int> & vI)
{
	if (!pgLegend)
	...
	if (vI.GetSize() == 1)
	{
		wksAdd.Columns(1).SetLongName("Отрицательный ионный ток");
		wksAdd.Columns(3).SetLongName("Положительный ионный ток");
		...
		dsX.SetSize(2);
		dsX[0] = v[0];
		v = sheetParamsData.Columns(1).GetDataObject(); /* x of Y == 0 (OX crossing point) */
		dsX[1] = v[vI[0] - 1];
		//dsX[1] = (abs(v[v.GetSize()-1]) - abs(v[0])) / 2;
		
		linfit(sheetFitData.Columns(0).GetDataObject(), sheetFitData.Columns(vI[0]).GetDataObject(), 0.1, 1, v);
		...
		v = sheetParamsData.Columns(1).GetDataObject(); /* x of Y == 0 (OX crossing point) */
		dsX[0] = v[vI[0] - 1];
		//dsX[0] = (abs(v[v.GetSize()-1]) - abs(v[0])) / 2;
...
int add_sublines_to_graph_doubleprobe(GraphPage & gp,
									  Page & pgLegend)
{
	Worksheet wks = pgLegend.Layers(1);
	
	/* Ii- line */
	Curve cIminus(wks, 0, 1);
	gp.Layers(0).AddPlot(cIminus, IDM_PLOT_LINE);
	
	/* Ii+ line */
	Curve cIplus(wks, 2, 3);
	gp.Layers(0).AddPlot(cIplus, IDM_PLOT_LINE);
	
	DataPlot dpIminus = gp.Layers(0).DataPlots(gp.Layers(0).DataPlots.Count() - 2),
			 dpIplus = gp.Layers(0).DataPlots(gp.Layers(0).DataPlots.Count() - 1);
	
	Tree trAdd;
	trAdd = dpIminus.GetFormat(FPB_ALL, FOB_ALL, TRUE, TRUE);
	
	trAdd.Root.Line.Width.dVal = 1.1;
	trAdd.Root.Line.Color.nVal = 3;
	
	dpIminus.ApplyFormat(trAdd, true, true);
	dpIplus.ApplyFormat(trAdd, true, true);
...
int reprocess_single_segment_doubleprobe(int begin,
									     int end,
									     int colnum,
									     ...
									     Page & pgOrigData)
{
	...
	int err = OriginMakeOne(3, vPila, vSignal, vAdd, vResultData, vFiltedData, vDiffedData, vParameters);
	if (err < 0)
		return err;
...
int reprocess_doubleprobe()
{
	...
	for (i = 0; i < vI.GetSize(); ++i)
	{
		...
		int err = reprocess_single_segment_doubleprobe(begin,
										               end,
										               vI[i],
										               sheetOrigData,
										               sheetFitData,
